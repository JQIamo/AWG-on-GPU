...
This is the test script for the streaming pathway benchmark. The DMA transfer is bypassed.
*/
        ...
        if (dynamic_total){
            ...
            while (!job_distirbuter_flag){
                ...
                for (int cnt = 0; cnt < dynamic_loopcount && !iskeypressed;cnt++){
                    if (0)  // ((dwError = spcm_dwSetParam_i32 (hCard, SPC_M2CMD, M2CMD_DATA_WAITDMA)) != ERR_OK)
                    {
                        if (dwError == ERR_TIMEOUT)
                            printf ("\n... Timeout %d\n",counterprint);
                        else
                        ...
                        AccelCombine<<< static_length/lThreadsPerBlock/2, lThreadsPerBlock >>>(sPointerPosition,saved_buffer_cuda,summed_buffer_cuda,dFreq_cuda,dDiff_cuda,dphi_cuda,moved_tweezers,moving_num);
                        cudaDeviceSynchronize();
